...
import { Briefcase, FileText, Handshake, Target } from 'lucide-react';
...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const interviewStages = ['Recruiter Call', 'Technical Interview', 'Final Round'];
    ...
    return { total, inInterview, offerRate, locationData };
  }, [applications]);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{stats.offerRate.toFixed(1)}%</div>
          <p className="text-xs text-muted-foreground">Based on total applications</p>
        </CardContent>
      </Card>
       <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
